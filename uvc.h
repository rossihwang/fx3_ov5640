...
/* Abort streaming event. This event flag is set when the UVC host sends down a request
   ...
   events can be considered as mutually exclusive.
       中止流式处理事件。当UVC主机发送一个请求（set_INTERFACE或CLEAR_FEATURE）指示应停止视频流时，将设置此事件标志。
       在设置此标志之前，将清除CY_FX_UVC_STREAM_EVENT事件，这两个事件可以视为互斥的。
 */
...
/* UVC VIDEO_CONTROL_REQUEST event. This event flag indicates that a UVC class specific
   ...
   UVC视频控制请求事件。此事件标志表示已收到针对视频控制接口的UVC类特定请求。固件服务后应立即清除。
 */
...
/* UVC VIDEO_STREAM_REQUEST event. This event flag indicates that a UVC class specific
   ...
   as soon as serviced by the firmware.
   UVC视频流请求事件。此事件标志表示已收到针对视频流接口的UVC类特定请求。固件服务后应立即清除。
 */
 ...
 * FX3 DMA重置事件。当FX3由于较慢的USB主机或帧计时器溢出而无法提交缓冲区时，会设置此事件。
 * 当设备以更高的分辨率和更高的fps传输时，USB带宽将饱和，主机将无法跟上。
 * 视频流可能工作几秒钟，然后设备将接收到提交缓冲区故障。
 * 由于某些原因，传感器/ISP也可能无法发送视频数据。
 * 在这种情况下，最好重置DMA并重新启动视频流，以便有连续的视频预览。
 */

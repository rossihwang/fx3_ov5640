...
static CyU3PEvent    glFxUVCEvent;                      /* Event group used to signal threads. �������̷߳����źŵ��¼��顣 */
...
CyFxUVCApplnInit (void)
{
    ...
    /* Initialize Gpio interface ��ʼ��GPIO�ӿ�*/
    ...
    /* CTL pins are restricted and cannot be configured using I/O matrix configuration function,CTL�����ܵ����ƣ�����ʹ��I/O�������ù��ܽ������ã�
     * must use GpioOverride to configure it ����ʹ��GpioOverride��������*/
    ...
    /* SENSOR_RESET_GPIO is the Sensor reset pin  SENSOR_RESET_GPIO�����Sensor�ĸ�λ����*/
    ...
    /* Initialize the P-port. ��ʼ������˿�*/
    pibclock.clkDiv      = 2;
    pibclock.clkSrc      = CY_U3P_SYS_CLK;
    pibclock.isDllEnable = CyFalse;
    pibclock.isHalfDiv   = CyFalse;

    apiRetStatus = CyU3PPibInit (CyTrue, &pibclock);//����ʱ��
    if (apiRetStatus != CY_U3P_SUCCESS)
    {
    ...
    CyFxUvcAppGpifInit ();//��ʼ��GPIF�ӿ�

    /* Register the GPIF State Machine callback used to get frame end notifications.ע�����ڻ�ȡ֡����֪ͨ��GPIF״̬���ص���
     * We use the fast callback version which is triggered from ISR context.����ʹ�ô�ISR�����Ĵ����Ŀ��ٻص��汾��
     */
    CyU3PGpifRegisterSMIntrCallback (CyFxGpifCB);//״̬���Ĵ����ж�
    ...
    /* Setup the Callback to Handle the USB Setup Requests ���ûص��Դ���USB��������*/
    CyU3PUsbRegisterSetupCallback (CyFxUVCApplnUSBSetupCB, CyFalse);

    /* Setup the Callback to Handle the USB Events ���ûص��Դ���USB�¼�*/
    CyU3PUsbRegisterEventCallback (CyFxUVCApplnUSBEventCB);

    /* Register a callback to handle LPM requests from the USB 3.0 host. ע��һ���ص�����������usb3.0������LPM����*/
    CyU3PUsbRegisterLPMRequestCallback (CyFxUVCAppLPMRqtCB);

    /* Register the USB device descriptors with the driver. ����������ע��USB�豸��������*/
    ...
    /* BOS and Device qualifier descriptors. BOS���豸�޶�����������*/
    ...
    /* Configure the video streaming endpoint. ������Ƶ���ս�㡣*/
    ...
    /* Configure the status interrupt endpoint.����״̬�ж϶˵㡣
       Note: This endpoint is not being used by the application as of now. This can be used in case ע�⣺Ӧ�ó���Ŀǰ��δʹ�ô��ս�㡣����������Է���һ
       UVC device needs to notify the host about any error conditions. A MANUAL_OUT DMA channel UVC�豸��Ҫ֪ͨ�����κδ��������һ���ֶ����DMA�ŵ�
       can be associated with this endpoint and used to send these data packets. ��������ս����������ڷ�����Щ���ݰ���
     */
    endPointConfig.enable   = 1;
  ...
  CyU3PTimerCreate(&UvcTimer, CyFxUvcAppProgressTimer, 0x00, glFrameTimerPeriod, 0, CYU3P_NO_ACTIVATE);
    ...
    /* Enable USB connection from the FX3 device, preferably at USB 3.0 speed. ��FX3�豸����USB���ӣ������USB 3.0�ٶȡ�*/
        ...
        uint32_t input)
{
    ...
    /*
       ...
       There is a provision for users to reset and/or turn OFF power to the sensor/Image signal processor (ISP).
       	�Ӵ�������USB������ʵ������ת����ͨ��DMA��GPIF�ص���ɵĹ��ܡ��߳�ֻ��������������/ֹͣ������
		CY_FX_UVC_STREAM_EVENT�¼���־��ָʾӦ����UVC��Ƶ����
		CY_FX_UVC_STREAM_ABORT_EVENT��ʾ��Ҫ��ֹ��Ƶ�������ֻ�е������յ�һ��CLEAR_FEATURE����ָʾ��ý�彫��ֹͣʱ�Żᷢ��������������·���г������ش���ʱ��
		CY_FX_UVC_DMA_RESET_EVENT��ʾ��Ҫ����DMA���ս�㻺�����Լ�����GPIF�����Ǵӵ�һ��״̬��������GPIF״̬�����豸����ʼ��ʽ������յ���һ֡�����Ƶ���������Ӧ�ã�������Ƕ���һЩ֡���ز�����Ƶ����Ӧ��û���⡣�������ͻȻֹͣ��Ƶ�����������һ�������û����顣
		��ע�⣬���ǲ������ύ������ʧ�ܺ�����������Ƶ������Ϊ����MAC OS����ġ�
		CY_FX_USB_SUSPEND_EVENT_HANDLERָʾ�豸�������͹���USB����ģʽ���û��������ú�/��رմ�����/ͼ���źŴ�������ISP���ĵ�Դ��
     */
    for (;;)
    {
        ...
        uint32_t input)
{
    ...
    for (;;)
    {
        /* Wait for a Video control or streaming related request on the control endpoint. �ȴ����ƶ˵��ϵ���Ƶ���ƻ���ý���������*/
        ...
        void)
{
    ...
    io_cfg.isDQ32Bit        = CyFalse;  // Enable DQ16bit to use uart on gpio[48:49] for debug

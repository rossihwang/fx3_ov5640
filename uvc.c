...
static CyU3PEvent    glFxUVCEvent;                      /* Event group used to signal threads. 用于向线程发出信号的事件组。 */
...
CyFxUVCApplnInit (void)
{
    ...
    /* Create UVC event group 创建UVC事件组*/
    ...
    /* Initialize Gpio interface 初始化GPIO接口*/
    ...
    /* CTL pins are restricted and cannot be configured using I/O matrix configuration function,CTL引脚受到限制，不能使用I/O矩阵配置功能进行配置，
     * must use GpioOverride to configure it 必须使用GpioOverride来配置它*/
    ...
    /* SENSOR_RESET_GPIO is the Sensor reset pin  SENSOR_RESET_GPIO是这个Sensor的复位引脚*/
    ...
    /* Initialize the P-port. 初始化这个端口*/
    pibclock.clkDiv      = 2;
    pibclock.clkSrc      = CY_U3P_SYS_CLK;
    pibclock.isDllEnable = CyFalse;
    pibclock.isHalfDiv   = CyFalse;

    apiRetStatus = CyU3PPibInit (CyTrue, &pibclock);//设置时钟
    if (apiRetStatus != CY_U3P_SUCCESS)
    {
    ...
    CyFxUvcAppGpifInit ();//初始化GPIF接口

    /* Register the GPIF State Machine callback used to get frame end notifications.注册用于获取帧结束通知的GPIF状态机回调。
     * We use the fast callback version which is triggered from ISR context.我们使用从ISR上下文触发的快速回调版本。
     */
    CyU3PGpifRegisterSMIntrCallback (CyFxGpifCB);//状态机的触发中断
    ...
    /* Setup the Callback to Handle the USB Setup Requests 设置回调以处理USB设置请求*/
    CyU3PUsbRegisterSetupCallback (CyFxUVCApplnUSBSetupCB, CyFalse);

    /* Setup the Callback to Handle the USB Events 设置回调以处理USB事件*/
    CyU3PUsbRegisterEventCallback (CyFxUVCApplnUSBEventCB);

    /* Register a callback to handle LPM requests from the USB 3.0 host. 注册一个回调来处理来自usb3.0主机的LPM请求。*/
    CyU3PUsbRegisterLPMRequestCallback (CyFxUVCAppLPMRqtCB);

    /* Register the USB device descriptors with the driver. 向驱动程序注册USB设备描述符。*/
    ...
    /* BOS and Device qualifier descriptors. BOS和设备限定符描述符。*/
    ...
    /* Configure the video streaming endpoint. 配置视频流终结点。*/
    ...
    /* Configure the status interrupt endpoint.配置状态中断端点。
       Note: This endpoint is not being used by the application as of now. This can be used in case 注意：应用程序目前尚未使用此终结点。这可以用来以防万一
       UVC device needs to notify the host about any error conditions. A MANUAL_OUT DMA channel UVC设备需要通知主机任何错误情况。一个手动输出DMA信道
       can be associated with this endpoint and used to send these data packets. 可以与此终结点关联并用于发送这些数据包。
     */
    endPointConfig.enable   = 1;
  ...
  CyU3PTimerCreate(&UvcTimer, CyFxUvcAppProgressTimer, 0x00, glFrameTimerPeriod, 0, CYU3P_NO_ACTIVATE);
    ...
    /* Enable USB connection from the FX3 device, preferably at USB 3.0 speed. 从FX3设备启用USB连接，最好是USB 3.0速度。*/
        ...
        uint32_t input)
{
    ...
    /*
       ...
       There is a provision for users to reset and/or turn OFF power to the sensor/Image signal processor (ISP).
       	从传感器到USB主机的实际数据转发是通过DMA和GPIF回调完成的功能。线程只负责检查流的启动/停止条件。
		CY_FX_UVC_STREAM_EVENT事件标志将指示应启动UVC视频流。
		CY_FX_UVC_STREAM_ABORT_EVENT表示需要中止视频流。这个只有当我们收到一个CLEAR_FEATURE请求，指示流媒体将被停止时才会发生，或者在数据路径中出现严重错误时。
		CY_FX_UVC_DMA_RESET_EVENT表示需要重置DMA和终结点缓冲区以及禁用GPIF。我们从第一个状态重新启动GPIF状态机，设备将开始流式传输接收到下一帧后的视频。对于相机应用，如果我们丢弃一些帧并重采样视频流，应该没问题。如果我们突然停止视频流，这可能是一个糟糕的用户体验。
		请注意，我们不会在提交缓冲区失败后重新启动视频流，因为这是MAC OS所需的。
		CY_FX_USB_SUSPEND_EVENT_HANDLER指示设备必须进入低功耗USB挂起模式。用户可以重置和/或关闭传感器/图像信号处理器（ISP）的电源。
     */
    for (;;)
    {
        ...
        uint32_t input)
{
    ...
    for (;;)
    {
        /* Wait for a Video control or streaming related request on the control endpoint. 等待控制端点上的视频控制或流媒体相关请求。*/
